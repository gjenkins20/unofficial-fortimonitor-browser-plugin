# BPA per-section delivery — design

FMN-145 planning ticket (2026-05-04). Companion to FMN-133 (the BPA wizard) and FMN-135 (frontend session-auth fetcher).
...
Today, `bpa-audit-handlers.js` stages the entire `{ inventory, analysis }` blob in `chrome.storage.session` under `bpa.lastRun`. The viewer reads it back as one chunk via `bpa:get-run-result`.
...
1. **Wizard section selector.** Add the pill row to the configure step; persist the selection on `store.sections`. Default to `["all"]`. Drives `payload.sections` in the `bpa:run-audit` message.
...
- Storage transport: same `bpa.lastRun` key, same handle dance.
