...
// Tenant Observations PDF export (FMN-136).
//
...
//
// Usage:
//   import { printReport } from '../../lib/observations-pdf.js';
...
import { getTabs, getVisibleTabs, csvCellValue } from '../ui/tenant-observations/viewer.js';
...
export function pdfFilename(customer) {
  ...
  const prefix = slug || 'tenant-observations';
  return `${prefix}_tenant-observations_${timestampPart()}.pdf`;
}
...
/**
 * Build a self-contained printable HTML document for the Observations report.
 *
...
export function buildPrintableHtml(ctx, { coverPage = false, customer = '', generatedAt = new Date().toISOString(), sections, flags } = {}) {
  ...
  parts.push(`<title>Tenant Observations${customer ? ` - ${esc(customer)}` : ''}</title>`);
...
export function printReport(ctx, options = {}) {
  return new Promise((resolve) => {
    ...
    iframe.setAttribute('data-test', 'observations-pdf-iframe');
