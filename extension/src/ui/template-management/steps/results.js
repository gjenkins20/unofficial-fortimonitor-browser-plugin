...
// Manage Server Templates — Step 4 (Results).
...
function toCsv(rows) {
  const header = ['input', 'server_id', 'display_name', 'plan', 'status', 'template_id', 'mapping_id', 'error'];
...
export function render({ container, store, navigate }) {
  ...
  rows.forEach((r, i) => {
    ...
    const detail = r.status === 'failed'
      ? (r.error || '')
      : r.status === 'skipped'
      ? (r.plan === 'error' ? (r.error || 'resolution error') : r.plan === 'skip' ? 'already in target state' : '')
      : r.plan === 'attach' && r.mappingId != null
      ? `attached — mapping id ${r.mappingId}`
      : r.plan === 'detach'
      ? 'detached (dissociate)'
      : r.plan === 'destroy'
      ? 'detached + metrics deleted'
      : '';
  ...
  csvBtn.addEventListener('click', () => {
    ...
    downloadBlob(`template-management-${stamp}.csv`, 'text/csv', toCsv(rows));
  });
