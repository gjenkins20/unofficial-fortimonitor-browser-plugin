...
// Tenant Observations viewer - 10-tab review surface (FMN-133, FMN-218).
//
...
export function buildTabCsv(tab, ctx, { generatedAt = new Date().toISOString(), customer = '' } = {}) {
  const lines = [];
  lines.push('# Unofficial FortiMonitor Toolkit - Tenant Observations');
...
export function tabFilename(tab, customer) {
  ...
  const prefix = slugCustomer || 'tenant-observations';
...
function combinedReadme(customer, generatedAt, tabs = getTabs()) {
  const lines = [
    'Unofficial FortiMonitor Toolkit - Tenant Observations',
...
export function combinedZipFilename(customer) {
  ...
  return `${prefix}_tenant-observations_${timestampPart()}.zip`;
}
...
// synthesis on top.
//
// FMN-218: the 'recommendations' tab was removed when the tenant observations shifted to
...
export function renderViewer({ root, store }) {
  ...
  // FMN-147: tenant_origin is set by runTenantObservations so the viewer can
