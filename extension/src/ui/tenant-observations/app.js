...
// Tenant Observations - top-level UI controller (FMN-133).
...
document.documentElement.dataset.toolMode = 'tenant-observations';
...
onEvent((event, payload) => {
  for (const cb of eventListeners) {
    try { cb(event, payload); } catch (err) { console.error('[tenant-observations event listener]', err); }
  }
});
const events = {
  on(cb) {
  ...
  emit(event, payload) {
    for (const cb of eventListeners) {
      try { cb(event, payload); } catch (err) { console.error('[tenant-observations synthetic emit]', err); }
    }
  }
};
...
function mountRoute(route, mod) {
  ...
  if (activeTeardown) {
    try { activeTeardown(); } catch (err) { console.error('[tenant-observations teardown]', err); }
    activeTeardown = null;
  }
  container.innerHTML = '';
