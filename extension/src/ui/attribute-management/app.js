...
// Top-level UI controller for the Manage Server Attributes (Bulk) tool.
// Mirrors src/ui/fabric-connection/app.js - hash router, in-memory store,
// fan-out event subscription.
...
document.documentElement.dataset.toolMode = 'attribute-management';

const store = {
  // From Start:
  operation: 'set',        // 'set' | 'remove'
  typeUrl: null,           // URL of the chosen server_attribute_type
  typeName: null,
  value: '',
  entries: [],             // raw pasted lines (names or numeric ids)
  attributeTypes: [],      // cached dropdown data
...
onEvent((event, payload) => {
  for (const cb of eventListeners) {
    try { cb(event, payload); } catch (err) { console.error('[attr-ui event listener]', err); }
  }
});
const events = {
  on(cb) {
...
function mountRoute(route, mod) {
  ...
  if (activeTeardown) {
    try { activeTeardown(); } catch (err) { console.error('[attr-ui teardown]', err); }
    activeTeardown = null;
  }
  container.innerHTML = '';
