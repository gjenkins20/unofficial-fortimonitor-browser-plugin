...
async function downloadSlot(slot) {
  const result = await sendMessage('observations-snapshots:export', { slot });
...
async function sendImport(envelope, { force }) {
  return sendMessage('observations-snapshots:import', { envelope, force: Boolean(force) });
}
...
async function renderDiff() {
  ...
  try {
    const diff = await sendMessage('observations-snapshots:diff', {
...
async function render() {
  ...
  let list = null;
  try {
    list = await sendMessage('observations-snapshots:list');
  } catch (err) {
    state.items = [];
