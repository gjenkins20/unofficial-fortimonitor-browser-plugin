...
} from '../src/lib/observations-frontend-fetcher.js';
...
// =============================================================================
// ObservationsFrontendFetcher.collect
// =============================================================================
...
test('ObservationsFrontendFetcher.collect: walks users and parses both fields', async () => {
  ...
  const fetcher = new ObservationsFrontendFetcher({ fetch });
  const users = [
...
test('ObservationsFrontendFetcher.collect: records error and skips user with no contact_info', async () => {
  let calls = 0;
...
test('ObservationsFrontendFetcher.collect: empty users list -> empty result, no requests', async () => {
...
test('ObservationsFrontendFetcher.collect: per-user 500 records error and continues', async () => {
  let n = 0;
...
test('ObservationsFrontendFetcher.collect: HTML response on first user is fatal (auth failure)', async () => {
...
test('ObservationsFrontendFetcher.collect: keys by user id even when only url is present', async () => {
...
test('ObservationsFrontendFetcher.collect: respects AbortSignal', async () => {
  ...
  const fetcher = new ObservationsFrontendFetcher({ fetch, signal: ac.signal });
  await assert.rejects(
    () => fetcher.collect([
      v2User({ userId: 1, contactId: 1001 }),
      v2User({ userId: 2, contactId: 1002 }),
...
// =============================================================================
// ObservationsFrontendFetcher.collectTemplateConfigs
// =============================================================================
...
test('ObservationsFrontendFetcher.collectTemplateConfigs: walks templates and parses each (FMN-135)', async () => {
  const fetch = createFetchMock(async (url) => {
...
test('ObservationsFrontendFetcher.collectTemplateConfigs: keys by template url id when id is missing', async () => {
  const captured = [];
  const fetch = createFetchMock(async (url) => {
    captured.push(url);
...
test('ObservationsFrontendFetcher.collectTemplateConfigs: HTML response on first template is fatal', async () => {
  const fetch = createFetchMock(async () => htmlResponse('<!DOCTYPE html>...spa shell...'));
  const fetcher = new ObservationsFrontendFetcher({ fetch });
  await assert.rejects(
...
test('ObservationsFrontendFetcher.collectTemplateConfigs: per-template 500 records error and continues', async () => {
  let n = 0;
  const fetch = createFetchMock(async () => {
    n++;
    if (n === 2) return jsonResponse('boom', { status: 500 });
