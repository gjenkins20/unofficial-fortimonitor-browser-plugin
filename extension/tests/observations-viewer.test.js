...
import { getTabs, getVisibleTabs, buildTabCsv, csvEscape, tabFilename, buildCombinedZipEntries, combinedZipFilename } from '../src/ui/tenant-observations/viewer.js';
...
test('tabFilename: blank customer falls back to "tenant-observations" prefix', () => {
  const tab = getTabs().find((t) => t.id === 'incidents');
  assert.match(tabFilename(tab, ''), /^tenant-observations_incidents_\d{8}\.csv$/);
});
...
test('buildTabCsv: Raw Counts tab emits a section header + table', () => {
  ...
  assert.match(csv, /^# Unofficial FortiMonitor Toolkit - Tenant Observations/);
...
test('combinedZipFilename: pattern is {customer}_tenant-observations_{YYYYMMDD}.zip', () => {
  assert.match(combinedZipFilename('Acme Corp'), /^acme-corp_tenant-observations_\d{8}\.zip$/);
});

test('combinedZipFilename: blank customer falls back to generic prefix', () => {
  assert.match(combinedZipFilename(''), /^tenant-observations_tenant-observations_\d{8}\.zip$/);
});
...
test('buildCombinedZipEntries: emits one CSV per visible tab + a README (FMN-218: 10 tabs + 1 README)', () => {
  ...
  assert.match(entries[0].content, /Tenant Observations/);
...
// FMN-218: the prior "Recommendations" tab was removed when the Tenant Observations
