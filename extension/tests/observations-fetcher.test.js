...
// =============================================================================
// createObservationsFetch (composed)
// =============================================================================

test('createObservationsFetch paces every attempt including retries', async () => {
  let now = 0;
  const sleeps = [];
  const fakeSleep = async (ms) => { sleeps.push(ms); now += ms; };
  let calls = 0;
...
// =============================================================================
// ObservationsFetcher: end-to-end inventory walks
// =============================================================================
...
test('ObservationsFetcher.collectInventory returns all 20 top-level keys plus trending and details', async () => {
  ...
  const fetcher = new ObservationsFetcher({ client });
...
test('ObservationsFetcher.collectInventory: 404 from a list endpoint becomes empty array, not an error', async () => {
...
test('ObservationsFetcher.collectInventory: 405 from a list endpoint is silently skipped (FMN-133 status_page)', async () => {
...
test('ObservationsFetcher.collectInventory: 401 from any list endpoint fails the whole collection', async () => {
  const routes = makeStandardRoutes();
...
test('ObservationsFetcher.collectInventory: 500 from a list endpoint records to errors[] and continues', async () => {
  const routes = makeStandardRoutes();
...
test('ObservationsFetcher.collectInventory: deep mode collects per-server data', async () => {
  const routes = makeStandardRoutes({
    servers: [
...
test('ObservationsFetcher.collectInventory: deep mode honors maxServers cap', async () => {
  const routes = makeStandardRoutes({
    servers: [
...
test('ObservationsFetcher.collectInventory: AbortSignal abort halts collection', async () => {
  const ctl = new AbortController();
  let calls = 0;
  ...
  const client = new PanoptaClient({ apiKey: 'k', fetch });
  const fetcher = new ObservationsFetcher({ client, signal: ctl.signal });
  await assert.rejects(
    () => fetcher.collectInventory(),
...
test('ObservationsFetcher.collectInventory: paginated active-outage logs are walked, capped at 50', async () => {
...
test('ObservationsFetcher.collectInventory: onProgress events fire for endpoint lifecycle', async () => {
  const events = [];
  const fetch = routeFetch(makeStandardRoutes());
  const client = new PanoptaClient({ apiKey: 'k', fetch });
  const fetcher = new ObservationsFetcher({ client, onProgress: (e) => events.push(e) });
...
test('ObservationsFetcher.collectInventory ["all"] preserves today\'s full crawl (FMN-149)', async () => {
...
test('ObservationsFetcher.collectInventory ["user-activity"] only walks /user (FMN-149)', async () => {
  const { routes, hits } = countingRoutes(makeStandardRoutes());
  const client = new PanoptaClient({ apiKey: 'k', fetch: routeFetch(routes) });
  const fetcher = new ObservationsFetcher({ client });
...
test('ObservationsFetcher.collectInventory ["incidents"] walks /outage + trending block, nothing else (FMN-149)', async () => {
  const { routes, hits } = countingRoutes(makeStandardRoutes({
    outages: [{ id: 5001, active: false }]
  }));
  const client = new PanoptaClient({ apiKey: 'k', fetch: routeFetch(routes) });
...
test('ObservationsFetcher.collectInventory ["template-recommendations","monitoring-policy"] shares fetched lists without duplication (FMN-149)', async () => {
  const { routes, hits } = countingRoutes(makeStandardRoutes({
...
test('ObservationsFetcher.collectInventory ["instance-analysis"] enables deep dive even without operator deep flag (FMN-149)', async () => {
  const seenUrls = [];
  const baseRoutes = [
...
test('ObservationsFetcher constructor rejects non-PanoptaClient inputs', () => {
  assert.throws(() => new ObservationsFetcher({}), /requires a PanoptaClient/);
  assert.throws(() => new ObservationsFetcher({ client: { getJson: () => {} } }), /requires a PanoptaClient/);
});
