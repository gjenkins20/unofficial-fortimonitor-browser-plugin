...
// FMN-164: unit tests for the observations-snapshots:* handler set. Covers the
// two estimate branches (last-run wins / 180s default) and the in-flight
// pickup path for observations-snapshots:status.

import { test } from 'node:test';
import assert from 'node:assert/strict';
import { createObservationsSnapshotHandlers } from '../src/background/observations-snapshot-handlers.js';
import { createStorageMock } from './fixtures/chrome-mocks.js';

function makeHandlers({ local, session } = {}) {
  return createObservationsSnapshotHandlers({
...
// =============================================================================
// observations-snapshots:estimate
// =============================================================================

test('estimate: no prior snapshot -> default 180s, basedOn:default', async () => {
  const handlers = makeHandlers();
  const out = await handlers['observations-snapshots:estimate']();
...
test('estimate: last-snapshot duration is returned verbatim', async () => {
  const local = createStorageMock({
    // Matches STORAGE_KEY in extension/src/lib/observations-snapshots.js.
    'fm:tenantObservationSnapshots': {
      current: {
...
// =============================================================================
// observations-snapshots:status - in-flight pickup
// =============================================================================

test('status: idle state -> runInFlight false, runStartedAt null', async () => {
  const handlers = makeHandlers();
  const out = await handlers['observations-snapshots:status']();
...
test('status: in-flight pickup from persisted session storage', async () => {
  ...
  const session = createStorageMock({
    'fmn.observationsSnapshot.runState': { startedAt },
  });
...
// =============================================================================
// FMN-161: observations-snapshots:export
// =============================================================================
...
test('export: current slot returns filename + JSON contents containing the snapshot', async () => {
  const local = createStorageMock({
    'fm:tenantObservationSnapshots': { current: snapFixture(), previous: null },
  });
  const handlers = makeHandlers({ local });
  const out = await handlers['observations-snapshots:export']({ slot: 'current' });
...
test('export: previous slot is selectable; empty slot returns ok:false reason:empty-slot', async () => {
  ...
  const handlers = makeHandlers({ local });
  const previousOut = await handlers['observations-snapshots:export']({ slot: 'previous' });
...
test('export: unknown slot value falls through to "current"', async () => {
  ...
  const handlers = makeHandlers({ local });
  const out = await handlers['observations-snapshots:export']({ slot: 'totally-bogus' });
  assert.equal(out.ok, true);
  assert.equal(out.slot, 'current');
});

// =============================================================================
// FMN-161: observations-snapshots:import
// =============================================================================
...
test('import: lands snapshot in previous slot when slot is empty', async () => {
  const local = createStorageMock({
    'fm:tenantObservationSnapshots': { current: snapFixture({ takenAt: '2026-05-12T10:00:00.000Z' }), previous: null },
  });
  const handlers = makeHandlers({ local });
  const importedAt = '2026-05-01T08:00:00.000Z';
  const out = await handlers['observations-snapshots:import']({
  ...
  const slots = local.__raw()['fm:tenantObservationSnapshots'];
...
test('import: refuses to overwrite an existing previous without force flag', async () => {
  ...
  const local = createStorageMock({
    'fm:tenantObservationSnapshots': { current: snapFixture(), previous: existingPrev },
  });
  const handlers = makeHandlers({ local });
  const incoming = snapFixture({ takenAt: '2026-04-01T00:00:00.000Z' });
  const out = await handlers['observations-snapshots:import']({ envelope: envelopeFor(incoming) });
...
test('import: future formatVersion rejected with wrong-format-version', async () => {
  ...
  const out = await handlers['observations-snapshots:import']({ envelope: bad });
  assert.equal(out.ok, false);
...
test('round-trip: exported snapshot imports back identically into previous slot', async () => {
  ...
  const exportOut = await exportHandlers['observations-snapshots:export']({ slot: 'current' });
  ...
  const importOut = await importHandlers['observations-snapshots:import']({
  ...
  const diffOut = await importHandlers['observations-snapshots:diff']();
...
// =====================================================================
// Phase 2.3: observations-snapshots:list + diff-by-id
// =====================================================================

test('list: empty store returns ok:true with empty items', async () => {
  const handlers = makeHandlers();
  const out = await handlers['observations-snapshots:list']();
  assert.equal(out.ok, true);
...
test('diff: with baselineId + currentId pair from history, computes diff', async () => {
  ...
  const out = await handlers['observations-snapshots:diff']({ baselineId: 'snap-h0', currentId: 'snap-c' });
  assert.equal(out.ok, true);
...
test('diff: with unknown id returns ok:false reason:unknown-id', async () => {
  ...
  const handlers = makeHandlers({ local });
  const out = await handlers['observations-snapshots:diff']({ baselineId: 'nope', currentId: 'also-nope' });
  assert.equal(out.ok, false);
...
test('diff: default (no payload) still diffs current vs previous', async () => {
  ...
  const handlers = makeHandlers({ local });
  const out = await handlers['observations-snapshots:diff']();
  assert.equal(out.ok, true);
...
test('get-config: defaults to 10 when nothing stored', async () => {
  const handlers = makeHandlers();
  const out = await handlers['observations-snapshots:get-config']();
  assert.equal(out.ok, true);
...
test('set-max: applies clamped value and persists', async () => {
  const local = createStorageMock();
  const handlers = makeHandlers({ local });
  const out = await handlers['observations-snapshots:set-max']({ maxSnapshots: 5 });
  assert.equal(out.ok, true);
  assert.equal(out.maxSnapshots, 5);
  const get = await handlers['observations-snapshots:get-config']();
  assert.equal(get.maxSnapshots, 5);
});

test('set-max: rejects non-finite input', async () => {
  const handlers = makeHandlers();
  const out = await handlers['observations-snapshots:set-max']({ maxSnapshots: 'a lot' });
  assert.equal(out.ok, false);
...
test('clear-all: wipes the store, list returns empty after', async () => {
  const local = createStorageMock({
    'fm:tenantObservationSnapshots': { current: snapFixture(), previous: snapFixture() },
  });
  const handlers = makeHandlers({ local });
  await handlers['observations-snapshots:clear-all']();
  const list = await handlers['observations-snapshots:list']();
...
test('status: in-flight in-memory (same-SW-session) returns the runtime start time', async () => {
  ...
  const takePromise = handlers['observations-snapshots:take']({}).catch(() => {});
