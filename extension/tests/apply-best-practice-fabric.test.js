...
import * as action from '../src/lib/bulk-actions/apply-best-practice-fabric.js';
...
test('action is registered in the bulk-actions registry', () => {
  ...
  assert.ok(ids.includes('apply-best-practice-fabric'));
  assert.equal(getAction('apply-best-practice-fabric').id, 'apply-best-practice-fabric');
});
...
const VALID_REC = {
  ...
  policy_proposal: {
    name: 'Apply Best-Practice FortiGate template',
    clauses: [
...
test('commit creates the policy + attaches the template on first target', async () => {
  ...
  assert.equal(out.policy.name, 'Apply Best-Practice FortiGate template');
...
test('commit skips policy creation when a ruleset with the same name already exists', async () => {
  const { panopta, fortimonitor, createCalls } = makeMockClients({
    existingRulesets: [{ id: 7777, name: 'Apply Best-Practice FortiGate template', latest_version: 1, config: { rules: [] } }]
  });
  const sharedState = new Map();
  const out = await action.commit(
