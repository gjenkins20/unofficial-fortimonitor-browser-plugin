...
// FMN-136 - tenant observations PDF print-document builder tests.
//
...
import { buildPrintableHtml, pdfFilename } from '../src/lib/observations-pdf.js';
import { getTabs } from '../src/ui/tenant-observations/viewer.js';
...
test('pdfFilename: pattern is {customer}_tenant-observations_{YYYYMMDD}.pdf', () => {
  assert.match(pdfFilename('Acme Corp'), /^acme-corp_tenant-observations_\d{8}\.pdf$/);
});

test('pdfFilename: blank customer falls back to generic prefix', () => {
  assert.match(pdfFilename(''), /^tenant-observations_tenant-observations_\d{8}\.pdf$/);
});
