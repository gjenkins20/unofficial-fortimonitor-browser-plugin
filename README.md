...
| **Ask Claude** | FortiMonitor v2 API key + Anthropic API key | Beta · Prototype (v0.7) | In-plugin Claude chat with tool use against a curated ~10-12 read-only FortiMonitor v2 endpoints (servers, outages, agent resources, fabric connections, templates, server groups) plus a single gated write (`acknowledge_outage`). Uses *your* Anthropic API key — cost exposure is direct, no rate limit. Prompt-caches the tool definitions. See [`docs/mcp-chat-prototype.md`](docs/mcp-chat-prototype.md) for scope, tool subset, and expand/kill criteria. |
