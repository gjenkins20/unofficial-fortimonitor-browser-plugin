...
// Tenant Observations viewer harness verification (FMN-133).
//
// Loads docs/harnesses/tenant-observations-viewer.html (a static fixture that
// imports viewer.js + observation-analyzers and renders against a synthetic
...
test.describe('Tenant Observations viewer harness (FMN-133)', () => {
  ...
  test('Combined-report ZIP button downloads a .zip with the customer-prefixed filename', async ({ extensionContext }) => {
    ...
    const download = await downloadPromise;
    expect(download.suggestedFilename()).toMatch(/^acme-corp-harness_tenant-observations_\d{8}\.zip$/);
    await page.close();
  });
