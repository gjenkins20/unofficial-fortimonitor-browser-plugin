...
// Apply Stock Fabric Templates action.
//
...
//
// Run: npx playwright test tests/e2e/fmn-196-stock-fabric-templates.spec.js
...
async function openConfigureWithFabricTargets(page, extensionId, targets, { stubResponses } = {}) {
  ...
  await page.evaluate(async (targets) => {
    ...
    mod.store.actionId = 'apply-stock-fabric-templates';
...
test.describe('FMN-196: Configure step renders the per-profile table', () => {
    ...
    await openConfigureWithFabricTargets(page, extensionId, [
      ...
      stubResponses: {
        'bulk-composer:list-monitoring-policy-vocab': {
          rulesets: [{ id: 7777, name: 'Apply Stock FortiGate template', latest_version: 1, config: { rules: [] } }],
  ...
  // for 'apply-stock-fabric-templates').
  // -----------------------------------------------------------------
